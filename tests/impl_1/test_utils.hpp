...
namespace test {
  ...
  void ExpectMatricesNear(const Eigen::MatrixBase<Derived> &a, const Eigen::MatrixBase<Derived> &b, double tolerance)
  {
    ASSERT_EQ(a.rows(), b.rows());
    ASSERT_EQ(a.cols(), b.cols());
    for (int i = 0; i < a.rows(); i++)
      for (int j = 0; j < a.cols(); j++)
        ASSERT_NEAR(a(i, j), b(i, j), tolerance) << "Entry (" << i << ", " << j << ") did not meet the tolerance!";
  }

  void ExpectArraysNear(int n, const double *a, const double *b, double tolerance)
  {
    ASSERT_GT(n, 0);
    for (int i = 0; i < n; i++) { EXPECT_NEAR(a[i], b[i], tolerance) << "i = " << i; }
  }
