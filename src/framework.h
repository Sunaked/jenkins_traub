...
/* computes (a*b - c*d) with precision not worse than 1.5*(unit of least precision) suggested in Claude-Pierre
Jeannerod, Nicolas Louvet, and Jean-Michel Muller, "Further Analysis of Kahan's Algorithm for the Accurate Computation
 ...
 *
 * The polynomial is represented both in the form of roots, e.g. (x-roots[0])*(x-roots[1])*(quadratic polynomial with no
 * real roots), as well as by its coefficients, e.g. coefficients[2]*x^2 + coefficients[1]*x + coefficients[0]. The
 * highest-degree coefficient always equals 1.
 *
 ...
 *
 * \return The actual number of different real roots placed into the vector (roots) (complex roots are not placed
 * there). Negative return values may mean internal implementation error.
 */
template<typename fp_t>
inline int generate_polynomial_2(unsigned N_pairs_of_complex_roots,
  unsigned N_clustered_roots,
  unsigned N_multiple_roots,
  fp_t max_distance_between_clustered_roots,
  fp_t root_sweep_low,
  fp_t root_sweep_high,
  ...
  std::vector<fp_t> &coefficients)
{
  ...
  std::uniform_real_distribution<fp_t> rnr(
    root_sweep_low, root_sweep_high);// uniform random data generator for single roots
  std::uniform_real_distribution<fp_t> rnc(
    static_cast<fp_t>(0.0L), max_distance_between_clustered_roots);// uniform random data generator for root clusters
 ...
 *
 * The polynomial is represented both in the form of roots, e.g. (x-roots[0])*(x-roots[1])*(quadratic polynomial with no
 * real roots), as well as by its coefficients, e.g. coefficients[3]*x^3 + coefficients[2]*x^2 + coefficients[1]*x +
 * coefficients[0]. The highest-degree coefficient always equals 1.
 *
 * \tparam fp_t The floating point type.
 *
 * \param N_pairs_of_complex_roots How many pairs of complex conjugate roots to introduce.
 ...
 */
template<typename fp_t>
inline int generate_polynomial_3(unsigned N_pairs_of_complex_roots,
  unsigned N_clustered_roots,
  unsigned N_multiple_roots,
  fp_t max_distance_between_clustered_roots,
  fp_t root_sweep_low,
  ...
  std::vector<fp_t> &coefficients)
{
  ...
  } else if (N_clustered_roots == 3)// 3 clustered distinct roots
  {
    ...
    roots[1] = im =
      (re > root_mid_sweep ? roots[2] = u = (re - im - u), re - im : roots[2] = u = (re + im + u), re + im);
 ...
 *
 * The polynomial is represented both in the form of roots, e.g. (x-roots[0])*(x-roots[1])*(quadratic polynomial with no
 * real roots), as well as by its coefficients, e.g. (coefficients[4]=1)*x^4 + coefficients[3]*x^3 + coefficients[2]*x^2
 * + coefficients[1]*x + coefficients[0]. The highest-degree coefficient always equals 1.
 *
 * \tparam fp_t The floating point type.
 *
 * \param N_pairs_of_complex_roots How many pairs of complex conjugate roots to introduce.
 ...
 */
template<typename fp_t>
inline int generate_polynomial_4(unsigned N_pairs_of_complex_roots,
  unsigned N_clustered_roots,
  unsigned N_multiple_roots,
  fp_t max_distance_between_clustered_roots,
  fp_t root_sweep_low,
 ...
 *
 * The polynomial is represented both in the form of roots, e.g. (x-roots[0])*(x-roots[1])*(quadratic polynomial with no
 * real roots), as well as by its coefficients, e.g. (coefficients[n]=1)*x^n + ... + coefficients[4]*x^4 +
 * coefficients[3]*x^3 + coefficients[2]*x^2 + coefficients[1]*x + coefficients[0]. The highest-degree coefficient
 * always equals 1.
 *
 * \tparam fp_t The floating point type.
...
int generate_polynomial(unsigned P,
  ...
  default:
    ...
    */
    ...
    for (i = 0; i < P; ++i) {
      ...
      for (int j = P - 2; j >= 0; --j) {
        // std::cout << "j:" << j << "\n";
        // std::cout << "-coeff[j+1]:" << -big_coeffs[j+1] << "; roots[i]:" << big_roots[i] << "; " << "coeff[j]: " <<
        // big_coeffs[j]; std::cout << "; fma: " << std::fma(-big_coeffs[j+1].ToDouble(), big_roots[i].ToDouble(),
        // big_coeffs[j].ToDouble());
        big_coeffs_new[j] = ttmath::Big<exponent, mantissa>(
          std::fma(-big_coeffs[j + 1].ToDouble(), big_roots[i].ToDouble(), big_coeffs[j].ToDouble()));
    ...
    for (i = 0; i < N_pairs_of_complex_roots; ++i) {
      ...
      for (int j = P - 2; j >= 0; --j) {
        big_coeffs_new[j] = ttmath::Big<exponent, mantissa>(std::fma(
          big_coeffs[j].ToDouble(), c1, std::fma(big_coeffs[j + 1].ToDouble(), c2, big_coeffs[j + 2].ToDouble())));
      }
 ...
 *
 * Root orderings play no role. For each entry in (roots_ground_truth), the closest entry in (roots_to_check) is found
 * and corresponding distance found. Among such distances, the largest will be stored to (max_deviation).
 *
 * \tparam fp_t The floating point type.
 *
 ...
 * \param max_absolute_error Here the greatest among the smallest deviations of the roots in (roots_to_check) and
 * (roots_ground_truth) will be placed. \param max_relative_error The maximum relative error.
 *
...
{
  int rv = (N_roots_to_check < N_roots_ground_truth)
  ...
  for (int j = 0; j < size; j++)
    for (int i = 0; i < size; i++) {
      long double absLoc =
        std::abs((long double)(roots_ground_truth[i]) - (long double)(roots_to_check[(i + j) % size]));
      abs = std::min(absLoc, abs);
      rel = std::min(std::abs((long double)(absLoc + std::numeric_limits<fp_t>::epsilon())
                              / (long double)(std::max(roots_to_check[(i + j) % size], roots_ground_truth[i])
