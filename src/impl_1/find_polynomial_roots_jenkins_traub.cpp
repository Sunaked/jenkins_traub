...
#define M_PI 3.14159265358979323846264338327950288
  ...
  void SyntheticDivisionAndEvaluate(const VectorXd &polynomial, const double x, VectorXd *quotient, double *eval)
  {
    ...
    for (int i = 1; i < polynomial.size() - 1; i++) { (*quotient)(i) = polynomial(i) + (*quotient)(i - 1) * x; }
    ...
    VectorXd *remainder)
  {
    ...
    // simple.
    if (quotient->size() == 1) {
      *remainder = polynomial.tail<2>() - polynomial(0) * quadratic_divisor.tail<2>();
      return;
    }
    ...
    for (int i = 2; i < polynomial.size() - 2; i++) {
      (*quotient)(i) =
        polynomial(i) - (*quotient)(i - 2) * quadratic_divisor(2) - (*quotient)(i - 1) * quadratic_divisor(1);
    }

    const VectorXd::ReverseReturnType &creverse_quotient = quotient->reverse();
    (*remainder)(0) = polynomial.reverse()(1) - quadratic_divisor(1) * creverse_quotient(0)
                      - quadratic_divisor(2) * creverse_quotient(1);
    (*remainder)(1) = polynomial.reverse()(0) - quadratic_divisor(2) * creverse_quotient(0);
  }

  // Determines whether the iteration has converged by examining the three most
  // recent values for convergence.
  template<typename T> bool HasConverged(const T &sequence)
  {
    const bool convergence_condition_1 = std::abs(sequence(1) - sequence(0)) < std::abs(sequence(0)) / 2.0;
    const bool convergence_condition_2 = std::abs(sequence(2) - sequence(1)) < std::abs(sequence(1)) / 2.0;
  ...
  template<typename T> bool HasRootConverged(const std::vector<T> &roots)
  {
  ...
  private:
    ...
    void UpdateKPolynomialWithQuadraticShift(const VectorXd &polynomial_quotient,
    ...
    ConvergenceType ApplyFixedShiftToKPolynomial(const std::complex<double> &root, const int max_iterations);
    ...
    bool ApplyVariableShiftToKPolynomial(const ConvergenceType &fixed_shift_convergence,
    ...
    bool ApplyQuadraticShiftToKPolynomial(const std::complex<double> &root, const int max_iterations);

    // Applies a linear shift to the K-polynomial to determine a single real root.
    // Return true if a root was successfully found.
    bool ApplyLinearShiftToKPolynomial(const std::complex<double> &root, const int max_iterations);
  ...
  bool JenkinsTraubSolver::ApplyQuadraticShiftToKPolynomial(const std::complex<double> &root, const int max_iterations)
  {
    ...
    // zero index is the convergence value from 2 iterations ago, the first
    // index is from one iteration ago, and the second index is the current value.
    VectorXd polynomial_quotient, polynomial_remainder, k_polynomial_quotient, k_polynomial_remainder;
    ...
    for (int i = 0; i < max_iterations; i++) {
      ...
      // iterations to help convergence.
      poly_at_root = std::abs(a_ - roots[0].real() * b_) + std::abs(roots[0].imag() * b_);
  ...
  bool JenkinsTraubSolver::ApplyLinearShiftToKPolynomial(const std::complex<double> &root, const int max_iterations)
  {
    ...
    double real_root = (root - EvaluatePolynomial(polynomial_, root) / EvaluatePolynomial(k_polynomial_, root)).real();
    ...
    for (int i = 0; i < max_iterations; i++) {
      ...
      k_polynomial_ =
        AddPolynomials(deflated_k_polynomial, -k_polynomial_at_root / polynomial_at_root * deflated_polynomial);
  ...
  void JenkinsTraubSolver::RemoveZeroRoots()
  {
    ...
    const VectorXd::ReverseReturnType &creverse_polynomial = polynomial_.reverse();
    ...
    const VectorXd &k_polynomial_quotient)
  {
    ...
    k_polynomial_ = AddPolynomials(
      coefficient_q_k * k_polynomial_quotient, MultiplyPolynomials(linear_polynomial, polynomial_quotient));
  ...
  VectorXd JenkinsTraubSolver::ComputeNextSigma()
  {
    ...
    const VectorXd::ReverseReturnType &creverse_k_polynomial = k_polynomial_.reverse();
    const VectorXd::ReverseReturnType &creverse_polynomial = polynomial_.reverse();

    const double b1 = -creverse_k_polynomial(0) / creverse_polynomial(0);
    const double b2 = -(creverse_k_polynomial(1) + b1 * creverse_polynomial(1)) / creverse_polynomial(0);
    ...
    const double c1 = c_ * c_ + u * c_ * d_ + v * d_ * d_ + b1 * (a_ * c_ + u * b_ * c_ + v * b_ * d_) - c4;
